...
const DISCIPLINAS = [
    "Arte", "Ensino Religioso", "Língua Portuguesa", "Matemática", 
    "História", "Ciências", "Língua Inglesa", "Geografia", "Educação Física"
];
...
/**
 * VISÃO GERAL DE NOTAS (SOMA DIRETA) COM ATUALIZAÇÃO DE CORES
 */
function openVerNotas() {
    ...
    ALUNOS.forEach(aluno => {
        ...
        const cAnual = totalAnual >= 60.00 ? 'nota-alta' : 'nota-baixa';
        ...
        tr.innerHTML = `
            ...
            <td class="${cAnual}" style="font-size:0.9rem;">${totalAnual.toFixed(2)}</td>
        `;
        corpo.appendChild(tr);
    });
...
/**
 * OPERAÇÃO CENTRAL DE FECHAMENTO GLOBAL
 * BLOQUEIA TODAS AS MATÉRIAS DE UMA VEZ E ABRE O PRÓXIMO SE O ANTERIOR ESTIVER FECHADO
 */
function renderFechamentoGlobalScreen() {
    ...
    // Atualiza o estado visual dos passos na tela
    for (let b = 1; b <= 4; b++) {
    ...
    if (currentActiveBim <= 4) {
        ...
        btn.innerHTML = `<i class="fas fa-lock-open"></i> Fechar ${currentActiveBim}º Bimestre Global (Bloquear Tudo)`;
        ...
        btnContainer.innerHTML = `
            <div class="info-alert" style="border-left-color: var(--success); background-color: rgba(16,185,129,0.1); color: var(--success);">
                <i class="fas fa-graduation-cap"></i> Ano Letivo de 2026 encerrado com sucesso! Todos os relatórios estão consolidados.
            </div>
        `;
    }
}
...
function executeGlobalClosureProcedure(bimestreParaFechar) {
    const msgConfirm = `ATENÇÃO PROFESSORA!\nDeseja fechar o ${bimestreParaFechar}º Bimestre de TODAS as matérias simultaneamente?\n\nEsta ação congelará as notas atuais e abrirá o próximo período. Não será possível reverter no aparelho sem restaurar backups.`;
    ...
    ALUNOS.forEach((aluno, index) => {
        ...
        for (let b = 1; b <= 4; b++) {
            ...
            tableBody.push([
                ...
                { content: finalBimVal.toFixed(2).replace('.', ','), styles: { fontStyle: 'bold', fillColor: [224, 242, 254], textColor: [37, 99, 235] } }
            ]);
        }
        ...
        tableBody.push([
            ...
            { content: totalAcumuladoGeral.toFixed(2).replace('.', ','), styles: { fontStyle: 'bold', fillColor: [16, 185, 129], textColor: [255, 255, 255], fontSize: 10 } }
        ]);
...
/*******************************************************************************
 * NOVO MÓDULO: BOLETIM INDIVIDUAL
 ******************************************************************************/

/**
 * Renderiza a lista de todos os alunos cadastrados na tela de boletim individual
 */
...
/**
 * Retorna uma ficha de rendimento consolidada do aluno para todas as matérias
 */
function obterFichaRendimentoAluno(aluno) {
    ...
    DISCIPLINAS.forEach(m => {
        ...
        } else if (ficha[m].totalAnual >= 50.00) {
            ficha[m].situacao = "Recuperação";
        } else {
...
/**
 * Constrói uma página de boletim oficial unificado no documento jsPDF fornecido
 */
function adicionarPaginaBoletim(doc, aluno, imgLogo) {
    // Paleta de cores do Brasão de Abre Campo:
    // Azul Escuro: [12, 44, 92] (#0C2C5C)
    // Dourado: [212, 175, 55] (#D4AF37)
    // Cinza Claro: [248, 250, 252] (#F8FAFC)
    // Preto: [15, 23, 42]
    ...
    doc.autoTable({
        ...
        didParseCell: function (data) {
            if (data.section === 'body') {
                // Colorização condicional inteligente: Azul para aprovado, Vermelho para abaixo da média
                ...
                // Campo Total e Média Geral (Critério: Total >= 60.00 pts, Média >= 15.00 pts)
                if (data.column.index === 5) {
                    const val = parseFloat(data.cell.raw.replace(',', '.'));
                    if (val < 60.00) data.cell.styles.textColor = [220, 38, 38];
                    else data.cell.styles.textColor = [16, 185, 129]; // Verde
                }
                ...
                // Situação por Extenso
                if (data.column.index === 7) {
                    if (data.cell.raw === "Aprovado") {
                        data.cell.styles.textColor = [16, 185, 129];
                    } else if (data.cell.raw === "Recuperação") {
    ...
    // Bloco Inferior de Assinaturas (Alinhamento Horizontal Seguro em A4)
    const lineY = 252;
...
/**
 * Gera e baixa o PDF individual de um estudante selecionado
 */
...
/**
 * Gera um arquivo PDF consolidado com todos os alunos (um por página)
 */
function exportarTodosBoletinsPDF() {
    ...
    doc.save(`boletins_unificados_800_2026.pdf`);
}
